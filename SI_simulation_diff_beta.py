...
from utils.utils import get_d_matrix, refactor_d_for_SI_simulation, get_populations_proportions, norm_d_to_one
from utils.run_simulations import run_full_SI_simulation

groups = 2
d_base, mean_age, norm_factor = get_d_matrix(groups, norm_to_one_meeting=False)
populations_proportions = get_populations_proportions(d_base)  # np.ones(d.shape[0])
...
if __name__ == '__main__':
    ...
    for base_beta in [.1, .2, .3, .4, .5, .6]:
        beta = base_beta / 12.5
        d, beta = norm_d_to_one(d_base,  beta)
        ...
        risk_l = RiskInitializer().age_to_risk_exponential_func(mean_age, number_of_simulations)/3
        ...
        data.to_pickle(f'test_run_{max_d_itr}_{groups}_{base_beta}_{now}.pickle')
        print('stop')
